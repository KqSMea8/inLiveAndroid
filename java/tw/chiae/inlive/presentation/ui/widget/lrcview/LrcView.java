...
public class LrcView extends View {
    ...
    private void init(AttributeSet attrs) {
        ...
        mTextSize = ta.getDimension(R.styleable.LrcView_lrcTextSize, LrcUtils.sp2px(getContext(), 16));
        ...
        mNormalColor = ta.getColor(R.styleable.LrcView_lrcNormalTextColor, 0xFFDDDDDD);
        mCurrentColor = ta.getColor(R.styleable.LrcView_lrcCurrentTextColor, 0xFFDDDDDD);
        mLabel = ta.getString(R.styleable.LrcView_lrcLabel);
        mLabel = TextUtils.isEmpty(mLabel) ? "暫無歌詞" : mLabel;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        // 画当前行
       ;
        mPaint.setColor(Color.parseColor(mLrcEntryList.get(mCurrentLine).getColor()));
        //mPaint.setTextSize(getHeight()*1/5+5);
        //Log.i("RayTest"," h:"+(getHeight()*1/5+5));
       ...
       /* float upY = currY;
        ...
        if(mCurrentLine+1< mLrcEntryList.size()){
            mPaint.setColor(Color.parseColor(mLrcEntryList.get(mCurrentLine+1).getColor()));
            mPaint.setTextSize(mTextSize-10);
            mPaint.setAlpha(90);
            drawText(canvas, mLrcEntryList.get(mCurrentLine+1).getStaticLayout(), downY);
        }
    ...
    private void onLrcLoaded(List<LrcEntry> entryList) {
        if (entryList != null && !entryList.isEmpty()) {
            ...
            for(LrcEntry lrc : entryList){
                //Log.i("RayTest",lrc.getText());
            }
        }
        if (hasLrc()) {
            initEntryList();
