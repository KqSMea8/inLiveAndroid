...
public class SharePreferenceManager {
    ...
    private static final String CACHED_PROFILE_KEY = "CachedProfileKey"; //本地照片 （大）

    public static void setCachedProfilePath(String path) {
        if (null != sp) {
            sp.edit().putString(CACHED_PROFILE_KEY, path).apply();
        }
    }

    public static String getCachedProfilePath() {
        if (null != sp) {
            return sp.getString(CACHED_PROFILE_KEY, null);
        }
        return "default";
    }
}
