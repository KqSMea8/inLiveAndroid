...
import tw.chiae.inlive.presentation.ui.photoselect.PhotoSelectActivity;
...
import tw.chiae.inlive.util.RealPathUtil;
...
public class ChatActivity extends BaseActivity implements View.OnClickListener, View.OnTouchListener,
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mLayoutBar = (RelativeLayout)findViewById(R.id.jmui_menu_title_bar);
        ...
        fromimg=intent.getParcelableExtra("fromimg");
        String address=intent.getStringExtra("address");
        String where=intent.getStringExtra("t");
        mTargetId = intent.getStringExtra(TARGET_ID);
        Log.i("RayTest","mTargetId:"+mTargetId+"   "+name);
        ...
        if (!TextUtils.isEmpty(mTargetId)) {
            ...
            if (mConv != null) {
                ...
                L.e("RayTest","他的名字1"+userInfo.getNickname()+userInfo.getUserName()+name);
                L.e("lw","他的名字1"+userInfo.getNickname()+userInfo.getUserName()+name);
                if (!name.equals(null)&&!name.equals("")){
                    mChatView.setChatTitle(name);
                }else {
                    L.e("RayTest","他的名字1a"+userInfo.getNickname()+userInfo.getUserName()+name);
                ...
                UserInfo userInfo = (UserInfo)mConv.getTargetInfo();
                L.e("RayTest","他的名字2"+userInfo.getNickname()+userInfo.getUserName()+name);
                L.e("RayTest","他的名字2b"+userInfo.getNickname()+userInfo.getUserName()+name);
                L.e("lw","他的名字2"+userInfo.getNickname()+userInfo.getUserName()+name);
                if (!name.equals(null)&&!name.equals("")){
                    mChatView.setChatTitle(name);
            ...
            Bitmap img = BitmapFactory.decodeFile("/storage/emulated/0/mypic_data/yangmi.jpg",new BitmapFactory.Options());
            if (where.equals("1")) {
                mChatAdapter = new MsgListAdapter(mContext, mTargetId, mTargetAppKey, longClickListener, img, fromimg, address);
            }else{
                if (fromimg==null){
                    mChatAdapter = new MsgListAdapter(mContext, mTargetId, mTargetAppKey, longClickListener, img);
                }else {
                    mChatAdapter = new MsgListAdapter(mContext, mTargetId, mTargetAppKey, longClickListener, img, fromimg);
                }
            }
        } else {
            mIsSingle = false;
    ...
    public void onClick(View v) {
        if (v.getId() == IdHelper.getViewID(mContext, "jmui_return_btn")) {
            ...
            UserInfo userInfo = (UserInfo)mConv.getTargetInfo();
            try {
            if (userInfo.getUserName().startsWith("user")) {
                String name =userInfo.getUserName();
                name=name.replace("user","");
                L.i("lw", "用户信息" + name);
                startActivity(OtherUserActivity.createIntent(ChatActivity.this,
                        Integer.parseInt(name), false));
            }else {
                startActivity(OtherUserActivity.createIntent(ChatActivity.this,
                        Integer.parseInt(userInfo.getUserName()), false));
//            startChatDetailActivity(mTargetId, mTargetAppKey, mGroupId);
            ...
            if (!FileHelper.isSdCardExist()) {
                ...
                intent.setClass(this, mImagePickerActivity.class);
                ...
                intent.putExtra(ImagePickerActivity.INTENT_EXTRA_FOLDER_TITLE, "相簿");
                intent.putExtra(ImagePickerActivity.INTENT_EXTRA_IMAGE_TITLE, "請選取圖片");
    ...
    private void handleSelectImgRefresh(Intent data){
        final Conversation conv = mConv;
        ArrayList<Image> images = data.getParcelableArrayListExtra(ImagePickerActivity.INTENT_EXTRA_SELECTED_IMAGES);
        StringBuilder sb = new StringBuilder();
        int[] msgsa = new int[images.size()];
        Intent intent = new Intent();
        for (int i = 0, l = images.size(); i < l; i++) {
            ImageContent imageContent = null;
            try {
                imageContent = new ImageContent(new File(images.get(i).getPath()));
                Message msg = conv.createSendMessage(imageContent);

                Log.i("RayTest",i+" getId: "+msg.getId());
                msgsa[i]=msg.getId();
                Log.i("RayTest",i+":"+images.get(i).getPath());
            } catch (FileNotFoundException e) {
                e.printStackTrace();
            }
            intent.putExtra(MsgIDs, msgsa);
...
//        mChatAdapter.getFromImg().recycle();
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        Log.i("RayTest","resultCode:"+resultCode +"  requestCode:"+requestCode);
        ...
        if (requestCode == REQUEST_CODE_TAKE_PHOTO) {
            ...
            handleSelectImgRefresh(data);
            //如果作为UIKit使用,去掉以下几段代码
        } else if (requestCode == RESULT_CODE_CHAT_DETAIL) {
            if (!mIsSingle) {
        ...
        } else if (requestCode == RESULT_CODE_FRIEND_INFO) {
            if (mIsSingle) {
                ...
                View.OnClickListener listener = new View.OnClickListener() {
                    ...
                    public void onClick(View v) {
                        if (v.getId() == IdHelper.getViewID(mContext, "jmui_copy_msg_btn")) {
                            ...
                            mDialog.dismiss();
                        }   else if (v.getId() == IdHelper.getViewID(mContext, "jmui_hear_msg_btn")) {
                            TextView hear= (TextView)v.findViewById(R.id.jmui_hear_msg_btn);
                            AudioManager audioManager = (AudioManager)getSystemService(Context.AUDIO_SERVICE);
                                if(hear.getText().toString().equals(getResources().getString(R.string.jmui_haerd))) {
                                    L.e("lw","进来 1");
                                    audioManager.setSpeakerphoneOn(true);
                                    audioManager.setMode(AudioManager.MODE_NORMAL);
                                    hear.setText(R.string.jmui_haer);
                                } else if(hear.getText().toString().equals(getResources().getString(R.string.jmui_haer))){
                                    L.e("lw","进来 2");
...
//                                    audioManager.setMode(AudioManager.MODE_IN_CALL);
                                    ...
                                    //设置为通话状态
                                    setVolumeControlStream(AudioManager.STREAM_VOICE_CALL);
                                    hear.setText(R.string.jmui_haerd);
                                    L.e("lw","进来 2"+hear.getText().toString());
                                }
                            mDialog.dismiss();
                        } else {
                ...
                View.OnClickListener listener = new View.OnClickListener() {
                    ...
                    public void onClick(View v) {
                        if (v.getId() == IdHelper.getViewID(mContext, "jmui_delete_msg_btn")) {
                            mConv.deleteMessage(publicMsgId);
                            mChatAdapter.removeMessage(publicPosition);
                            mDialog.dismiss();
                        } else if (v.getId() == IdHelper.getViewID(mContext, "jmui_forward_msg_btn")) {
                            mDialog.dismiss();
                        }
