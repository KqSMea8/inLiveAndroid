...
public class EditAvatarActivity extends BaseActivity {
    ...
    public static Intent createIntent(Context context, String avatarPath){
        ...
        Log.i("RayTest","EXTRA_AVATAR:"+avatarPath);
        return intent;
    }
    ...
    protected void init() {
        ...
        reflashProfile();
        Log.i("RayTest","mAvatarPath:"+mAvatarPath);
        ...
        subscribeClick(btnTakePhoto, new Action1<Void>() {
            ...
            public void call(Void aVoid) {
                ...
                try {
                    ...
                    Log.i("RayTest","btnTakePhoto :"+ path);
...
*/
                    ...
                    startActivityForResult(intent,RESULT_REQUST_CODE_CAMERA);
                      ...
                      Log.i("RayTest","Exception e:"+e.getMessage());
//                    Toast.makeText(ImpromptuActivity.this, "没有找到储存目录",Toast.LENGTH_LONG).show();
                ...
                try{
                    ...
                    JMessageClient.updateUserAvatar(new File(path), new BasicCallback() {
                        ...
                        public void gotResult(int status, final String desc) {
                            if (status == 0) {
                                Log.i("RayTest","updateUserAvatar ok!!"+path );
                                //finish();
                            }
                        }
                    });
    ...
    private void updateAvatar(final String path){
        Log.i("RayTest","updateAvatar path:"+path);
...
//                                }
                                ...
                                public void onSuccess(BaseResponse<String> response) {
                                    Log.i(LOG_TAG, "Upload success!");
                                    SharePreferenceManager.setCachedProfilePath(path);
                                    reflashProfile();
                                }
                            })
                            ;
...
//                        }
                ...
                .subscribe(new Action1<String>() {
                    ...
                    public void call(final String s) {
                        ...
                        Log.i("RayTest","on post result: "+ s);
    ...
    private void reflashProfile() {
        ...
        String path = SharePreferenceManager.getCachedProfilePath();
        if(!TextUtils.isEmpty(path) || path!=null){
            File localfile = new File(path);
            final Uri uri = Uri.parse("file://"+localfile.getAbsolutePath());
            Log.i("RayTest","getAbsolutePath path:"+localfile.getAbsolutePath());
            FrescoUtil.frescoResize(uri,
                    metrics.widthPixels, metrics.widthPixels,
                    draweeAvatar);
        }else{
            if(!TextUtils.isEmpty(mAvatarPath)) {
                FrescoUtil.clearCache(SourceFactory.wrapPathToUri(mAvatarPath));
            /*FrescoUtil.frescoResize(SourceFactory.wrapPathToUri(mAvatarPath+"?systime="+getSystemTime()),
                    metrics.widthPixels, metrics.widthPixels,
                    draweeAvatar);*/
                FrescoUtil.frescoResize(SourceFactory.wrapPathToUri(mAvatarPath ),
                        metrics.widthPixels, metrics.widthPixels,
                        draweeAvatar);
                Log.i("RayTest","set mAvatarPath1:");
            }
            ...
            if(mAvatarUri != null) {
                FrescoUtil.frescoResize(mAvatarUri,
                        metrics.widthPixels, metrics.widthPixels,
                        draweeAvatar);
                Log.i("RayTest","set mAvatarPath2:");
            }
        }
    }
