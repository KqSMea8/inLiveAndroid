 ...
 */
public class MediaRecorderActivity extends AppCompatActivity implements MediaRecorderBase.OnErrorListener, OnClickListener, MediaRecorderBase.OnPreparedListener,
        MediaRecorderBase.OnEncodeListener, MediaRecorderBase.OnMergeListener, CreateViewDialogFragment.dialogCallback {
    ...
    public static final String MV_BT_NAME = "mvBtPath";
    ...
    private boolean record_start_count = false;
    private boolean prepareDelRecord = false;
    private HeadSetReceiver receiver;

    private boolean PermissionError = false;  // false : 權限錯誤發生
    private boolean needShowPauseMsg = false;
    ...
    public static Intent createIntent(Context context, String btName, String apiUrl, int starID, boolean isAutoPlay ) {
        //_starID = starID;
        Intent it = new Intent(context, MediaRecorderActivity.class);
        it.putExtra(MV_BT_NAME, btName);
        it.putExtra(APIURL, apiUrl);
        it.putExtra(STAR_ID, starID);
        it.putExtra(AUTO_PLAY,isAutoPlay);
        return it;
    }
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        PermissionError = CheckPermissions();
        if(PermissionError){
            loadIntent();
            loadViews();
        }
...
//            Manifest.permission.MODIFY_AUDIO_SETTINGS,
    ...
    private boolean CheckPermissions() {
        ...
        int lacksCount = mPermissionsChecker.getLacksPermissions(PERMISSIONS).size();
        Log.i("RayTest","getLacksPermissions size:"+lacksCount);
        if (lacksCount>0) {
    ...
    private String getLrcPath() {
        String lrcPath = getIntent().getStringExtra(APIURL) + getIntent().getStringExtra(MV_BT_NAME) + ".lrc";
        return lrcPath;
    }

    private String getMVPath() {
        String mvPath = getIntent().getStringExtra(APIURL) + getIntent().getStringExtra(MV_BT_NAME) + ".mp4";
...
//        recording_state = RECORDING_LOADING_STATE;
    ...
    private void initMediaRecorder() {
        String key = "K_star_rec_" + _starID + "_" + LocalDataManager.getInstance().getLoginInfo().getUserId();
        if(mMediaRecorder!=null)
            mMediaRecorder = null;
        mMediaRecorder = new MediaRecorderSystem(this);
        mRebuild = true;
        ...
        Log.i("RayTest", "Path1:" + VCamera.getVideoCachePath() + key + " :" + FileUtils.checkFile(VCamera.getVideoCachePath() + key) + " ");
    ...
    private void fristStartMv(MediaPlayer mp) {
        ...
        Log.i("RayTest", "setImageResource recording_ready！！ 2");
        ...
        if(isAutoPlay){
            ...
            isAutoPlay=false;
             ...
             if (recording_state == RECORDING_FRIST_STATE) {
                 Log.i("RayTest","RECORDING_FRIST_STATE");
                 mVideoView.setVolume(0.1f, 0.1f);
    ...
    public void onResume() {
        ...
        if(needShowPauseMsg){
            dialogFragment.showMsgDialog(getSupportFragmentManager(),"提示","錄製中斷，請重新錄製。",CreateViewDialogFragment.TYPE_PAUSE_MEDIA_RECORD , false);
            needShowPauseMsg = false;
        }

        Log.i("RayTest","PermissionError:"+PermissionError);

        initreceiver();
        if(mMediaRecorder!=null){
            if(mMediaRecorder.camera == null)
                Log.i("RayTest","conResume camera null");
            else
                Log.i("RayTest","conResume camera OK");
...
/*        isPuase = false;
        ...
        if (recording_state == RECORDING_READY_STATE){
            if (mMediaRecorder != null)
                Log.i("RayTest", "mMediaRecorder release");
        ...
        if (mMediaRecorder == null) {
            ...
            String key = "K_star_rec_" + _starID + "_" + LocalDataManager.getInstance().getLoginInfo().getUserId();
            final String path = VCamera.getVideoCachePath() + key;
            ...
            Log.i("RayTest", "mMediaRecorder null 2");
            ...
            public void run() {
                ...
                try {
                    ...
                    if (mMediaRecorder == null) {
                        initMediaRecorder();
                        Log.i("RayTest","Check File");
                        String key = "K_star_rec_"+_starID;
                        final String path = VCamera.getVideoCachePath() + key;
                        if(FileUtils.checkFile(path)){
                            Log.i("RayTest","存在 "+path);
                        ...
                        mMediaRecorder.setSurfaceCallback(new MediaRecorderBase.SurfaceCallback() {
                            @Override
                            public void isSurfaceCreated() {
                                mMediaRecorder.setSurfaceCallback(null);
    ...
    private void showPermissionsDialog() {
        Log.i("RayTest","showPermissionsDialog");
        dialogFragment.showMsgDialog(getSupportFragmentManager(),"提示",getString(R.string.permissions_error),CreateViewDialogFragment.TYPE_SHOW_ERROR,false);
    }
    ...
    public void onPause() {
        ...
        isStartAnim = false;
        startTimeAnim = 1000;
        startAnimImg.setImageResource(R.drawable.start_second_three);
//        stopRecord();
        if (recording_state == RECORDING_RECORDING_STATE)
        {
            ...
            if(!isRecordComplete) {
                ...
                Log.i("RayTest","stopRecord4");

                stopRecord();

                needShowPauseMsg = true;
    ...
    private void showPermissionErrorMsg() {
...
/*    @Override
    public void onPause() {
        super.onPause();
        if(mMediaRecorder==null)
            Log.i("RayTest","mMediaRecorder null 1");
        else
            Log.i("RayTest","mMediaRecorder not null 1");
        isPuase = true;
        Log.i("mrl", "onPause");
        if (isfinish)
            return;
...
//        stopRecord();
        if (recording_state == RECORDING_RECORDING_STATE) {
            Log.i("RayTest", "onPause "+isRecordComplete);
            if (!isRecordComplete)
            {
                dialogFragment.showFinishFialog(getSupportFragmentManager(),"提示","錄製影片遭到暫停, 請重新錄製!",CreateViewDialogFragment.TYPE_PAUSE_MEDIA_RECORD);
       ...
       *//* if (!mReleased) {
        ...
        if(mMediaRecorder==null)
            Log.i("RayTest","mMediaRecorder null 3");
        else
            Log.i("RayTest","mMediaRecorder not null 3");
    }*/
    ...
    protected void onDestroy() {
        ...
        if (downloadListenerLrc != null)
            downloadListenerLrc = null;
        if (timer != null)
            timer.cancel();
        if (mVideoView != null) {
            mVideoView.stop();
    ...
    protected void onSaveInstanceState(Bundle outState) {
        ...
        Log.i("RayTest","onSaveInstanceState");
    }
    ...
    public void deletRecording(String msg) {
        ...
        if (mMediaObject != null && mMediaObject.getDuration() > 1) {
            ...
            dialogFragment.showMsgDialog(getSupportFragmentManager(),msg,getString(R.string.record_camera_exit_dialog_message),CreateViewDialogFragment.TYPE_RECORD_MEDIA,true);
    ...
    public void deletRecordingRecording(String msg) {
        if(prepareDelRecord)
            return;
        prepareDelRecord  = true;
        mSurfaceView.setVisibility(View.VISIBLE);
        if (mRecordDelete != null && mRecordDelete.isChecked()) {
            cancelDelete();
            prepareDelRecord  = false;
            return;
        }
        if (mMediaObject != null && mMediaObject.getDuration() > 1) {
            // 未转码
...
*/
            ...
            dialogFragment.showMsgDialog(getSupportFragmentManager(),msg,getString(R.string.record_camera_exit_dialog_message),CreateViewDialogFragment.TYPE_DEL_RECORD,true);
            return;
        }
    ...
    private void reCording() {
        ...
        mRecordController.setVisibility(View.VISIBLE);
        Log.i("RayTest", "setImageResource recording_ready！！ 3");
        mRecordController.setImageResource(R.drawable.recording_ready);
    ...
    public void onBackPressed() {
        if (isRecordComplete) {
            Log.i("RayTest", "仍會保存完整錄製作品？");
            dialogFragment.showMsgDialog(getSupportFragmentManager(), "提示", "您確認關閉頁面嗎？仍會保存完整錄製作品。", CreateViewDialogFragment.TYPE_FINISH_MEDIA_RECORD,true);
            //finish();
        }else {
            ...
            Log.i("RayTest", "您確認關閉頁面嗎？");
            dialogFragment.showMsgDialog(getSupportFragmentManager(), "提示", "您確認關閉頁面嗎？", CreateViewDialogFragment.TYPE_FINISH_MEDIA_RECORD,true);
    ...
    private void showStopRecordDialog() {
        dialogFragment.showMsgDialog(getSupportFragmentManager(),"提示","您確認關閉頁面嗎？將不會保留當前已錄製影片。",CreateViewDialogFragment.TYPE_FINISH_MEDIA_RECORD,true);
    }
    ...
    private void stopRecord() {
        ...
        if (mMediaRecorder != null) {
            Log.i("RayTest","mMediaRecorder null ");
...
//        mRecordDelete.setVisibility(View.VISIBLE);
        ...
        Log.i("RayTest", mMediaObject.getDuration() + " 結束了stopRecord " + mVideoView.getCurrentPosition() + "  " + RECORD_TIME_MAX);
        if (mMediaObject.getDuration() >= RECORD_TIME_MAX) {
            ...
            Log.i("RayTest", "setImageResource recording_ready！！ 1");
            mRecordController.setImageResource(R.drawable.recording_ready);
            if (recording_state != VIDEO_MEGER_PLAY)
                recording_state = RECORDING_READY_STATE;
//            mVideoView.seekTo(0);
    ...
    public void onClick(View v) {
        ...
        switch (id) {
            ...
            case R.id.title_next:// 停止录制
                ...
                Log.i("RayTest","mediaPath: "+mMediaObject.getPart(0).mediaPath);
...
//                        startRecord();
            ...
            case R.id.start_mergemv:
                Log.i("RayTest","start_mergemv "+mTestMergeVideo.isPlaying());
                if (!mTestMergeVideo.isPlaying()) {
                    mSurfaceView.setVisibility(View.INVISIBLE);
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case VIDEO_TIME_OVER:
                    Log.i("RayTest","stopRecord3");
                    stopRecord();
                    break;
                case TIME_SEND_FOUR:
                    Log.i("RayTest","TIME_SEND_FOUR");
                    if(!isTimeEnd) {
                        isTimeEnd = true;
                        stopAnim();
                    }
                ...
                case TIME_SEND_TWO:
                    ...
                    startAnimImg.setImageResource(R.drawable.start_second_two);
                    break;
                case TIME_SEND_THREE:
                    startAnimImg.setImageResource(R.drawable.start_second_one);
                    break;
            }
        }
    };
    ...
    public void onMergeComplete(String path) {
        ...
        Intent it = getIntent();
        if (it != null) {
            String fileName = getIntent().getStringExtra(MV_BT_NAME);
            String apiUrl = getIntent().getStringExtra(APIURL);
            Intent it_player = MediaPlayerActivity.createIntent(this, path, fileName, apiUrl, _starID);
   ...
   /*                 RelativeLayout.LayoutParams params = (RelativeLayout.LayoutParams) mRecordController.getLayoutParams();
    ...
    TimerTask timerTask = new TimerTask() {
        ...
        public void run() {
            ...
            Log.i("timeer", "跑起來" + isend + "  " + mVideoView.getMediaPlayer() + "   " + mVideoView.isPlaying() + "    " + mVideoView.getCurrentPosition() + "   [video Duration]:" + mVideoView.getDuration() + "  [obj Duration]:" + mMediaObject.getDuration() + " " + RECORD_TIME_MAX);
            /**
             * 刷新歌詞進度
             */
            if (mVideoView.isPlaying()) {
    ...
    private void downLoadMv(String btPath) {
        ...
        if (mDownloadRequest != null && mDownloadRequest.isStarted() && !mDownloadRequest.isFinished()) {
            ...
            Log.i("RayTest", "download");

            // 添加到队列，在没响应的时候让按钮不可用。這裡取消所有動作除了強制退出
        }
        Log.i("RayTest", "download end");
    }
////////////////
  /*  check MV Size*/

    private DowloadThread mDownloadThread = new DowloadThread();
    ...
    public void onOKDialogcheck(Bundle bundle) {
        ...
        switch (type){
            ...
            case CreateViewDialogFragment.TYPE_PAUSE_MEDIA_RECORD:
                Log.i("RayTest","TYPE_PAUSE_MEDIA_RECORD");
                reCording();
            ...
            case CreateViewDialogFragment.TYPE_MV_DOWNLOAD_ERROR:
                ...
                dialogFragment.dismiss();
                break;
            case CreateViewDialogFragment.TYPE_SHOW_ERROR:
                /*MediaRecorderActivity.this.finish();*/
                Log.i("RayTest","onBackPressed!!");
                dialogFragment.dismiss();
                if (mMediaObject != null)
                    mMediaObject.delete();
                finish();
            ...
            case CreateViewDialogFragment.TYPE_RECORD_MEDIA:
                reCording();
                break;
            case CreateViewDialogFragment.TYPE_DEL_RECORD:
                isPuase = false;
                startAnimImg.setImageResource(R.drawable.start_second_three);
                Log.i("RayTest","stopRecord1");
                stopRecord();
    ...
    private static class DowloadThread extends Thread {
        WeakReference<MediaRecorderActivity> mActivity;
        private String sName;

        @Override
        public void run() {

            int netMvLen = getFileSize(sName);
            String[] fileName = sName.split("/");
            String mvName = VCamera.getVideoCachePath() + fileName[fileName.length - 1];
            Log.i("RayTest", "mvName :" + mvName);
            File f = new File(mvName);

            if (f.exists()) {
                Log.i("RayTest", "localNet :" + f.length() + " mvNet:" + netMvLen);

                if (f.length() != netMvLen) {
                    f.delete();
                    mActivity.get().runOnUiThread(new Runnable() {
                        @Override
                        public void run() {

                            toastShort("檔案已更新，請重新進入合唱！");
        ...
        public void setMvPath(MediaRecorderActivity activity, String name) {
            mActivity = new WeakReference<>(activity);
            sName = name;
        }

        protected void toastShort(@NonNull String msg) {
            CustomToast.makeCustomText(mActivity.get(), msg, Toast.LENGTH_SHORT).show();
        }
    }


    private static int getFileSize(String CheckUrl) {
        URL url = null;
        int file_size = 0;
        try {
        ...
        Log.i("RayTest", "file size:" + file_size);
        return file_size;

    }


    private boolean CheckMvFile(final String sMvName) {
/*        if(Thread.State.NEW == mDownloadThread.getState()) {
            Log.d("RayTest", "mThread name: " + mDownloadThread.getName());
            mDownloadThread.setMvPath(sMvName,downloadhandler);
            mDownloadThread.start();
        }

        downloadhandler.sendEmptyMessage(1);*/
        mDownloadThread.setMvPath(this, sMvName);
        mDownloadThread.start();
        return true;
    }
...
////////////////
    ...
    private DownloadListener downloadListener = new DownloadListener() {
        ...
        public void onProgress(int what, int progress, long fileCount) {
            ...
            Log.i("RayTest", "當前進度" + progressVal+"   progress:"+progress);
            if (mVideoView != null)
    ...
    private class HeadSetReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent data) {
            try {
                if (data != null) {
                    //插入了耳机
                    Log.i("BroadcastReceiver",data.getAction());
                    if (data.getAction().equals(Intent.ACTION_HEADSET_PLUG)) {
                        int state = data.getIntExtra("state",-1);
                        Log.i("BroadcastReceiver","state : "+ state);
                        if(state == 0 && mVideoView.isPlaying()){
                            startAnimImg.setImageResource(R.drawable.start_second_three);
                            stopRecord();
                            dialogFragment.showMsgDialog(getSupportFragmentManager(),"提示","錄影期間請勿拔耳機或截圖。為了確保您合唱品質，請重新錄製。",CreateViewDialogFragment.TYPE_PAUSE_MEDIA_RECORD , false);
                        }
                    }
                }
