...
import static tw.chiae.inlive.presentation.ui.main.mergefilm.DaYuMediaPlayerActivity.VEDIOURL;
...
public class DaYuMediaPlayerModel {
    private final DaYuMediaPlayerPresenter presenter;
...
//            mTvResult.setText(message);
    ...
    public DaYuMediaPlayerModel(DaYuMediaPlayerPresenter mediaPlayerPresenter) {
    ...
    public String getRecordPath() {
        ...
        String key = "DaYu_rec_"+itData.getStringExtra(STAR_ID);
        String[] fileName = itData.getStringExtra(VEDIOURL).split("/");
        String[] filePath = fileName[fileName.length - 1].split(".mp4");
        String recordPath = VCamera.getVideoCachePath()+key+"/"+filePath[filePath.length - 1]+"/0.mp4";

        Log.i("RayTest","getRecordPath:"+recordPath);
        return recordPath;
    }
    ...
    public String getMvPath() {
        Log.i("RayTest","Modle getMvPath "+itData.getStringExtra(VEDIOURL));
        String[] fileName = itData.getStringExtra(VEDIOURL).split("/");
        //return VCamera.getVideoCachePath()+itData.getStringExtra(MV_NAME)+".mp4";
        return VCamera.getVideoCachePath()+fileName[fileName.length - 1];
    }

    public String getServerFile() {
        Log.i("RayTest","getServerFile:"+itData.getStringExtra(VEDIOURL));
        return itData.getStringExtra(VEDIOURL);
    }
    ...
    public boolean getNetWorkType(Context context) {
        ...
        if (networkInfo != null && networkInfo.isConnected()) {
            ...
            DaYuMediaPlayerActivity activity = (DaYuMediaPlayerActivity) context;
    ...
    public void downLoadMv() {
        ...
        if (mDownloadRequest != null && mDownloadRequest.isStarted() && !mDownloadRequest.isFinished()) {
            // 暂停下载。
            Log.i("RayTest","mDownloadRequest cancel");
            ...
            Log.i("RayTest","download folder :"+fileName[fileName.length - 1]);
    ...
    public boolean getStatus() {
        Log.i("RayTest","model:"+MvStatus+" RecordStatus"+RecordStatus );
    ...
    public int getStarID() {
        return Integer.parseInt(itData.getStringExtra(STAR_ID));
    }
}
